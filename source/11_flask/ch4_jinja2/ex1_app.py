# jinja2 템플릿 문법
# 1. 변수 {{var}} or {{var:filter}}
    # 기본 제공 필터 : lower, upper, capitalize(첫글자만 대문자), title(단어 첫글자만 대문자), length, trim
    #        형변환 : {{number|int}}
# 2. 제어구조 {%...%}
# 2-1. if
...
# 2-2. for
    # {% for var in vars %}
        ...
        # loop.index: 1부터, loop.index0: 0부터, loop.first: 첫번째면 True, loop.last: 마지막이면 True
    # {% endfor %}
...
def index():
    if request.method == 'GET':
        ...
        name = request.form['name'].strip()  # post방식 파라미터값
