...
SELECT INITCAP('welcome to oracle')
    FROM DUAL; --어절 첫문자만 대분자
SELECT INITCAP('WELCOM TO ORACLE')
    FROM DUAL; --어절 첫문자만 대분자
...
SELECT CONCAT(CONCAT('AB','CD'),CONCAT('EF','GH'))
    ...
    --SMITH는 MANAGER다 포멧으로 EMP테이블 데이터를 출력
...
SELECT SUBSTR('ORACLE', 4, 2)
    FROM DUAL; --시작 INDEX가 1, -가 붙으면 역순
SELECT SUBSTR('데이터베이스', 4, 2)
    FROM DUAL;
    
    --    SUBSTRB(STR, 시작바이트위치0, 바이트갯수) : STR을 시작부터 바이트갯수만큼 문자 추출
SELECT SUBSTRB('ORACLE', 4, 2) 
    FROM DUAL;
SELECT SUBSTRB('데이터베이스', 10, 6) --한글은 한글자가 3바이트
    FROM DUAL;

    -- 9월에 입사한 사원의 모든 필드 'RR/MM/DD')
SELECT *
    FROM EMP
    WHERE SUBSTR(HIREDATE,4,2) = '09'; --DATE형도 사용가능
...
SELECT LENGTHB('오라클')
    ...
    -- (5) LPAD(STR,자리수,채울문자) <- 문자열 왼쪽빈자리에 채울문자로 출력
    --     LPAD(STR,자리수) <- 문자열 왼쪽빈자리에 빈문자열로 출력
SELECT LPAD('ORACLE', 10, '*')
    FROM DUAL;
SELECT LPAD('ORACLE', 10)
    FROM DUAL;
    --     RPAD(STR,자리수,채울문자) <- 문자열 오른쪽빈자리에 채울문자로 출력
    --     RPAD(STR,자리수) <- 문자열 오른쪽빈자리에 빈문자열로 출력
SELECT RPAD('ORACLE', 10, '*')
    FROM DUAL;
SELECT RPAD('ORACLE', 10)
    FROM DUAL;

    --EX. 이름은 10문자까지 SAL은 5자리로
SELECT LPAD(ENAME,10,'-'), LPAD(SAL,5,'$')
    FROM EMP;
    
    --EX. 사번, 이름(S**** / W***...) -RPAD, SUBSTR, LENGTH
...
SELECT *
    FROM EMP
    WHERE HIREDATE LIKE '__/__/09';
    
    --EX. 9일이 아닌 날에 입사한 사원의 모든필드
SELECT *
    FROM EMP
...
SELECT LTRIM('    ORARCLE     ')
    FROM DUAL;     
    
SELECT RTRIM('    ORARCLE     ')
    FROM DUAL;        
    
    --(8) REPLACE(STR, 바꿔야할 문자, 바꿀 문자)
...
SELECT SYSDATEM, SYSDATE+1
    FROM DUAL;
SELECT TO_CHAR(SYSDATE+1, 'RR/MM/DD HH:MI')
    FROM DUAL;    
...
SELECT TRUNC(SYSDATE-TO_DATE('24/08/26 09:30', 'RR/MM/DD HH24:MI'))
    ...
    -- 현재부터 수료까지 몇일 남았나?
SELECT TRUNC(TO_DATE('25/03/07 18:00', 'RR/MM/DD HH24:MI') - SYSDATE) D
    FROM DUAL;
...
SELECT ENAME, HIREDATE, TRUNC(SYSDATE - HIREDATE) WORKDAY,
        TRUNC((SYSDATE - HIREDATE)/7) WORKWEEK,
        TRUNC((SYSDATE - HIREDATE)/365) WORKYEAR
    FROM EMP;    
    
    --(3) MONTHS_BETWEEN(날짜형1, 날짜형2) : 두 날짜형 데이터 간 개월수 (날짜형 1이 큰 시점날짜)    
SELECT ENAME, HIREDATE, TRUNC(MONTHS_BETWEEN(TO_DATE('25/03/07 18:00', 'RR/MM/DD HH24:MI'),SYSDATE))   MONTH
    FROM EMP;
    --EX. 수료시점까지 남은 개월수
SELECT TRUNC(MONTHS_DATE(TO_DATE('25/03/07 18:00', 'RR/MM/DD HH24:MI') - SYSDATE)) D
    FROM DUAL;    
...
SELECT ADD_MONTHS(SYSDATE, 3)
    ...
    --EX. 이름, 입사일, 수습기간종료(수습기간은 입사일로부터 6개월까지)
SELECT ENAME, HIREDATE, ADD_MONTHS(HIREDATE, 6)
    FROM EMP;
    
SELECT ADD_MONTHS(TO_DATE('24/08/31','YY/MM/DD'), 6)
    FROM DUAL;
...
SELECT TRUNC(SYSDATE, 'YEAR')
    FROM DUAL;    
    --EX. 이름, 입사일, 첫월급날(16일) : ROUND의 기준은 15일이면 이달 1일, 16이면 다음달 1일
...
SELECT HIREDATE, TO_CHAR(HIREDATE, 'DD') 
    FROM EMP; -- 일만 문자로 추출
-- (2) 계층별 출력
...
SELECT ENAME, HIREDATE, ROUND(MONTHS_BETWEEN(SYSDATE,TO_DATE(HIREDATE, 'RR/MM/DD'))) MONTH, SAL * (ROUND(MONTHS_BETWEEN(SYSDATE,TO_DATE(HIREDATE, 'RR/MM/DD'))))
FROM EMP;

--5. 모든 사원의 이름과 급여(15자리로 출력 좌측의 빈 곳은 “*”로 대치)를 출력
SELECT LPAD(ENAME, 15, '*'),LPAD(SAL, 15, '*')
    FROM EMP;
...
SELECT EMPNO, ENAME, TO_CHAR(HIREDATE, 'RR/MM/DD')
    FROM EMP
