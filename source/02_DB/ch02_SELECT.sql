...
--1. SELECT문 작성법 (실행 : CTRL + ENTER/블록잡고 F9)
SHOW USER;
SELECT * FROM TAB;
SELECT * FROM EMP; -- EMP테이블의 보든 열 (필드), 모든 행
SELECT * FROM DEPT; -- DEPT테이블의 보든 열 (필드), 모든 행
SELECT * FROM SALGRADE; -- SALGRADE테이블의 보든 열 (필드), 모든 행
--날짜 표기법 셋팅(RR/MM/DD)
...
SELECT EMPNO, ENAME, SAL, JOB 
    FROM EMP; -- EMP테이블의 EMPNO, ENAME, SAL, JOB 필드만 모든행
SELECT EMPNO, ENAME, COMM 
    FROM EMP;
SELECT EMPNO AS "사 번", ENAME AS "이름", SAL AS "급여" --필드명에 별칭을 사용
    FROM EMP;
SELECT EMPNO "사 번", ENAME "이름", SAL "급여" -- 별칭에 AS 생략가능
    FROM EMP;
SELECT EMPNO "사 번", ENAME 이름, SAL 급여 -- 별칭에 SPACE가 없는경우 " 생략가능
    FROM EMP;
...
SELECT * 
    ...
    --EX3. 연봉(SAL*12)이 2400이상인 직원 ENAME, SAL, 연봉(ANNUALSAL)
...
SELECT ENAME, SAL, SAL*12 ANNUALSAL
    FROM EMP
    WHERE SAL*12 >= 2400
    ORDER BY ANNUALSAL; --ORDER BY 에서는 필드의 별칭 사용 가능
...
SELECT EMPNO, ENAME, SAL, SAL*1.1 UPGRADESAL
    FROM EMP
    WHERE SAL*1.1 >= 3000;
    --EX1. 모든사원의 ENAME, SAL(급여), COMM(상여), 연봉(SAL*12 + COMM)
SELECT ENAME, SAL, COMM, SAL*12+COMM 연봉 -- 산술연산결과는 NULL을 포함하면 결과도 NULL
    FROM EMP;
    --NVL(NULL일수도 있는값, NULL이면 대체할값) : NULL일수도 있는 값과 NULL이면 대체할 값의 타입이 일치
SELECT ENAME, SAL, COMM, SAL*12+NVL(COMM, 0) ANNUALSAL
    FROM EMP;
    --MGR이 NULL이면 CEO라고 출력
...
SELECT ENAME || '은 ' || JOB
    FROM EMP;
    --EX1. 모든사원에 대해 ENAME ':' SAL '=XXX' 포멧으로 출력 (연봉 = SAL*12+COMM)
...
SELECT ENAME || '은 ' || JOB || ' 업무이고 연봉은' || (SAL*12+NVL(COMM,0)) || ' 이다' "FORMAT"
    FROM EMP;
...
SELECT *
    ...
    --(3) LIKE
    --필드 LIKE 포맷 : %(0글자 이상), _(한글자)를 포함한 패턴
    --EX. 이름이 M으로 시작하는 사원의 모든필드
SELECT *
    FROM EMP
    WHERE ENAME LIKE 'M%';
    --EX. 이름에 N이 포함된 사원의 모든 필드
SELECT *
    FROM EMP
    WHERE ENAME LIKE '%N%';
...
SELECT ENAME, SAL, HIREDATE
    FROM EMP
    ORDER BY SAL DESC, HIREDATE;    --급여 내림차순 정렬 (급여 같으면 입사순)
    
SELECT ENAME, SAL
    FROM EMP
    ORDER BY SAL DESC, HIREDATE DESC; --급여 같으면 신입순
    
SELECT * 
    FROM EMP ORDER BY ENAME;
    
SELECT ENAME, SAL*12 ANNUALSAL
    FROM EMP ORDER BY ANNUALSAL;
