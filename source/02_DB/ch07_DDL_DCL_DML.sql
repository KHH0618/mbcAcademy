...
DROP TABLE EMP04; --EMP의 특정 행 (서브쿼리의 WHERE절로 제한)
CREATE TABLE EMP04
    AS
...
DROP TABLE EMP05;
CREATE TABLE EMP05 -- EMP 구조만 추출
    AS
    SELECT * FROM EMP WHERE 0=1;
SELECT * FROM EMP05;


--2. 테이블 구조 변경 (ALTER TABLE 테이블명 ADD || MODIFY || DROP COLMN)
-- 필드 추가 (ADD)
SELECT * FROM EMP03;
ALTER TABLE EMP03 
...
--필드 수정 (MODIFY)
ALTER TABLE EMP03 MODIFY EMPNO VARCHAR2(4); --숫자데이터가 들어있으면 숫자로만 변경 가능
ALTER TABLE EMP03 MODIFY EMPNO NUMBER(5);
ALTER TABLE EMP03 MODIFY EMPNO NUMBER(4); --숫자는 줄이는게 불가
ALTER TABLE EMP03 MODIFY ENAME VARCHAR(100);
ALTER TABLE EMP03 MODIFY ENAME VARCHAR(10); --문자데이터 필드는 늘리거나 줄이는거 가능
SELECT MAX(LENGTH(ENAME)) FROM EMP;
ALTER TABLE EMP03 MODIFY ENAME VARCHAR(5); --문자데이터보다 작은 자리수는 불가능
ALTER TABLE EMP03 MODIFY (SAL VARCHAR2(10), COMM NUMBER(3)); -- NULL필드는 마음데로 수정가능
...
--3. 테이블삭제 (DROP TABLE 테이블명)
DROP TABLE EMP03;
DROP TABLE EMP01;
DROP TABLE DEPT01; --EMP01테이블에서 DEPT01테이블을 참조할 경워 EMP01을 삭제한 후 DEPT01테이블 살제가능

--4. 테이블에 모든 데이터 제거하기(TRUNCATE TABLE 테이블명)
...
-- 데이터 딕셔너리 뷰의 종류
    --(1) USER_XXX : 현계정이 소유하고 있는 객체 (테이블, 뷰, 인덱스, 제약조건)
        --USER_TABLES, USER_CONSTRAINTS, USER_ VIEWS, USER_INDEXES
SELECT * FROM USER_TABLES;
SELECT * FROM USER_CONSTRAINTS;
SELECT * FROM USER_VIEWS;
SELECT * FROM USER_INDEXES;

    --(2) ALL_XXX : 현 계정에서 접근 가능한 객체 (테이블, 뷰, 인덱스, 제약조건)
        --ALL_TABLES, ALL_CONSTRAINTS, ALL_ VIEWS, ALL_INDEXES
SELECT * FROM ALL_TABLES;

    --(3) DBA_XXX : DBA권한에서만 접근 가능한 객체
         --DBA_TABLES, DBA_CONSTRAINTS, DBA_ VIEWS, DBA_INDEXES
...
SELECT * FROM DEPT01;
 --EX. BOOK (ID = 11, NAME = 스포츠의 의학, 출판사는 한솔출판, 출판일은 오늘, 가격은 90000)
SELECT * FROM BOOK;
INSERT INTO BOOK VALUES (11,'스포츠의 의학','한솔출판',SYSDATE, 90000);

    --EX. BOOK (ID = 12, NAME = 스포츠 과학, 출판사는 NULL, 출판일은 오늘, 가격은 50,000)
INSERT INTO BOOK (BOOKID, BOOKNAME, RDATE, PRICE) VALUES (12, '스포츠 과학', SYSDATE, TO_NUMBER('50,000','9,999,999'));
...
-- 2. UPDATE 테이블명 SET 필드명1 = 값1,... WHERE 제약조건
DROP TABLE EMP01;
CREATE TABLE EMP01
    AS
...
SELECT * FROM EMP;
UPDATE EMP01 SET (HIREDATE,SAL) = (SELECT HIREDATE,SAL
                                    FROM EMP01
                                    WHERE UPPER(ENAME) = 'KING');
SELECT * FROM EMP01;
...
CREATE TABLE MY_DATA( ID NUMBER(4) PRIMARY KEY,
                      NAME VARCHAR2(10),
                      USERID VARCHAR2(30),
                      SALARY NUMBER(10,2));

--2. 생성된 테이블에 위의 도표와 같은 값을 입력하는 SQL문을 작성하시오.
INSERT INTO MY_DATA VALUES (1, 'Scott', 'sscott', TO_NUMBER('10,000.00','999,999.99'));
...
SELECT TO_CHAR(SALARY,'00,000.00') FROM MY_DATA;
...
DELETE FROM MY_DATA WHERE UPPER(NAME) = 'FORD';
COMMIT;
...
UPDATE EMP01 SET SAL = SAL*0.9
    WHERE UPPER(JOB) = 'MANAGER';

-- 제약조건
-- PRIMARY KEY : 테이블의 각 행을 유일한 값으로 식별하기 위한 필드
...
INSERT INTO EMP1 (EMPNO, ENAME, JOB, DEPTNO)
    VALUES (1001, 'HONG', 'MANAGER', 10); -- HIREDATE 에는 DEFAULT값 입력
SELECT * FROM EMP1;
...
INSERT INTO EMP1 (EMPNO, ENAME, JOB, SAL, COMM, DEPTNO)
    VALUES (1002, 'AAAA', 'MANAGER', 10, -8, 20);

INSERT INTO EMP1 (EMPNO, ENAME, JOB, HIREDATE, SAL, DEPTNO)
    VALUES (1003,'BBB', 'MANAGER', TO_DATE('24/01/01','RR/MM/DD'), 1000, 20);


-- DCL : 계정생성, 권한부여, 권한박탈, 계정삭제--
CREATE USER scott2 IDENTIFIED BY tiger;
GRANT CREATE SESSION TO scott2; --로그인 권한 부여
GRANT CREATE TABLE TO scott2; --테이블생성 권한 부여
GRANT ALL ON EMP TO scott2; --EMP테이블에 대한 모든 권한 부여
...
INSERT INTO BOOK
    VALUES ('100A01', 100, '철학자의 삷', '이젠출판', 2021);
INSERT INTO BOOK
...
SELECT BOOKNO, BOOKNAME, PUBLISHER, PUBYEAR, CATEGORYNAME,OFFICE_LOC
    ...
    WHERE BOOKCATEGORY.CATEGORYCODE = BOOK.CATEGORYCODE;
...
CREATE TABLE STUDENT(
    ...
    sNAME VARCHAR2(50),
    sSsCORE NUMBER(3) CHECK (sSsCORE BETWEEN 1 AND 100),
    mCORD NUMBER(2) REFERENCES MAJOR(mCORD)
);
...
SELECT sNO, sNAME, sSsCORE, M.mCORD, mNAME, mOFFICE
    FROM STUDENT S, MAJOR M
    WHERE S.mCORD = M.mCORD;
