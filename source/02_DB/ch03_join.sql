...
--1. EQUI JOIN (동등JOIN = 공통필드값이 일치되는 조건만 JOIN)
SELECT *
    FROM EMP, DEPT
...
SELECT ENAME, JOB, SAL, E.DEPTNO
    ...
    --EX. 부서번호가 10 이거나 20이고, 82년도에 입사한 직원들의 이름,업무,급여,근무지 (급여순)
...
SELECT ENAME, JOB, SAL, LOC
    ...
    --EX. NEW YORK에서 근무하는 사원의 이름과 급여를 출력
SELECT ENAME, SAL
    FROM EMP E, DEPT D
    WHERE E.DEPTNO = D.DEPTNO AND LOC = 'NEW YORK';
    
    --EX. ACCOUNTING 부서 소속 사원의 이름과 입사일 출력
...
SELECT ENAME, DNAME
    ...
    --EX. COMM이 NULL이 아닌 사원의 이름, 급여, 부서번호, 근무지를 출력
SELECT ENAME, SAL, D.DEPTNO, LOC, COMM
    FROM EMP E, DEPT D
    WHERE E.DEPTNO = D.DEPTNO
        AND COMM IS NOT null;
    
    --EX. COMM이 NULL이고 SAL이 1200 이상인 사원의 이름, 급여, 부서번호, 부서명 (부서명순, 급여 큰순)    
...
SELECT * 
    ...
    --EX. 모든 사원의 사번, 이름, 상사사번, 급여, 급여등급
SELECT EMPNO, ENAME, MGR, SAL, GRADE||'등급' "GRADE"
    FROM EMP, SALGRADE
    WHERE SAL BETWEEN LOSAL AND HISAL;
    
    --EX. COMM이 NULL이 아닌 사원의 이름, 급여, 급여등급, 부서번호, 부서명, 근무지 
SELECT ENAME, SAL, GRADE, D.DEPTNO, DNAME, LOC
    FROM EMP E, DEPT D, SALGRADE S
    WHERE E.DEPTNO = D.DEPTNO AND (SAL BETWEEN LOSAL AND HISAL);
    
    --EX. 이름 급여 입사일 급여등급 (신입순)
SELECT ENAME, SAL, HIREDATE, GRADE
    FROM EMP, SALGRADE
    WHERE SAL BETWEEN LOSAL AND HISAL
    ORDER BY HIREDATE DESC;
    
    --EX. 이름 급여 급여등급 연봉 부서명(부서명정렬, 부서명같으면 연봉순 정렬,연봉 = (SAL + COMM)*12)
SELECT ENAME, SAL, GRADE, (SAL + NVL(COMM,0))*12 YEARSAL, DNAME
    FROM EMP E, DEPT D, SALGRADE S
    WHERE E.DEPTNO = D. DEPTNO
        AND (SAL BETWEEN LOSAL AND HISAL)
    ORDER BY DNAME, YEARSAL;
    
    -- EX. 이름, 직책, 급여, 등급, 부서번호, 부서명(급여가 1000~3000사이. 정렬조건:부서번호순, 직책순, 급여큰순)
SELECT ENAME, JOB, SAL, GRADE, D.DEPTNO, DNAME
    ...
    -- EX. 이름, 급여, 등급, 입사일, 근무지(81년 입사한 직원만 등급 큰순)
...
SELECT W.ENAME || '의 상사는 ' || M.ENAME "MSG"
 ...
 SELECT * FROM EMP;   
    ...
    --EX. 매니저가 KING인 사원들의 이름과 직급 출력
SELECT W.ENAME, W.JOB, M.ENAME
    FROM EMP W, EMP M
    WHERE W.MGR = M.EMPNO
        AND M.ENAME = 'KING';

    --EX. SCOTT과 동등한 부서번호인 사원의 이름을 출력
SELECT W.ENAME
    FROM EMP W, EMP M
    WHERE W.DEPTNO = M.DEPTNO
        AND M.ENAME = 'SCOTT'
        AND W.ENAME != 'SCOTT';
        
    --EX. SCOTT과 동일한 지역에서 근무하는 사원의 이름을 출력
SELECT W.ENAME
    FROM EMP W,EMP M, DEPT WD, DEPT MD
    WHERE M.DEPTNO = MD.DEPTNO
        AND W.DEPTNO = WD.DEPTNO
        AND M.ENAME = 'SCOTT'
        AND MD.LOC = WD.LOC
        AND W.ENAME != 'SCOTT';
...
SELECT * FROM EMP, DEPT;
    ...
    --SELF JOIN에서의 OUTER JOIN
SELECT W.ENAME, W.MGR, M.EMPNO, M.ENAME
    FROM EMP W, EMP M
    WHERE W.MGR = M.EMPNO(+);
    
    --SMITH의 상사는 FORD다(상사가 없는 직원은 KING은 상사는 없다 라고 출력)
SELECT W.ENAME|| '상사는 '|| NVL(M.ENAME,'없') || '다'
    FROM EMP W, EMP M
    WHERE W.MGR = M.EMPNO(+);
...
--3. 이름, 급여, 업무, 직속상사명 . (상사가 없는 직원까지 전체 직원 다 출력.
    --상사가 없을 시 '없음'으로 출력)
SELECT W.ENAME,W.SAL,W.JOB, NVL(M.ENAME,'없다')
    FROM EMP W, EMP M
    WHERE W.MGR = M.EMPNO(+);
...
SELECT W.ENAME,W.SAL,W.DEPTNO,D.DNAME,D.LOC, NVL(M.ENAME,'없다')
    FROM EMP W, EMP M, DEPT D
    WHERE W.MGR = M.EMPNO(+)
        AND W.DEPTNO = D.DEPTNO;
...
SELECT W.ENAME,W.SAL,GRADE,D.DNAME, NVL(M.ENAME,'없다')
...
SELECT W.ENAME,W.SAL,GRADE,D.DNAME,(W.SAL+NVL(W.COMM,0))*12 YEALSAL, NVL(M.ENAME,'없다')
    FROM EMP W, EMP M, DEPT D, SALGRADE
    WHERE W.MGR = M.EMPNO(+)
        AND W.DEPTNO = D.DEPTNO
        AND W.SAL BETWEEN LOSAL AND HISAL;
...
--12. 위의 결과에서 상위 상사가 없는 모든 직원의 이름도 출력되도록 수정하시오(outer join)
  SELECT E1.ENAME, NVL(E2.ENAME,'KING'),NVL(E3.ENAME, 'KING')
    FROM EMP E1, EMP E2, EMP E3
    WHERE E1.MGR = E2.EMPNO(+)
        AND E2.MGR = E3.EMPNO(+); 
